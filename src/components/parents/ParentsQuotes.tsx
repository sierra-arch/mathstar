...
export default function ParentsQuotes() {
  return (
    <section className="relative py-40 px-6 lg:px-16 bg-[#FFF5F5] overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(252,165,165,0.50) 0%, rgba(220,38,38,0.16) 42%, transparent 68%)" }} />
      ...
      <div className="absolute -bottom-20 -left-20 w-[460px] h-[460px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(127,29,29,0.20) 0%, transparent 62%)" }} />
      <div className="absolute top-0 left-1/4 w-[450px] h-[450px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(254,242,242,0.90) 0%, transparent 65%)" }} />
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-20">
          ...
          >
            ...
            <br />
            <em className="not-italic italic text-[#DC2626]">are already seeing it.</em>
          </motion.h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {quotes.map((q, i) => (
            <motion.div
              key={i}
              className="bg-white rounded-3xl p-10 border border-[#DC2626]/8 shadow-sm flex flex-col"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: i * 0.1 }}
            >
              <div className="text-5xl text-[#DC2626]/15 font-serif leading-none mb-5">&ldquo;</div>
